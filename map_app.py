...
DEFAULT_LOCATION = (CENTER_LAT, CENTER_LON)
DEFAULT_ZOOM = ZOOM_START

# Thay tên file bạn vừa tạo ở Bước 2
GRAPHML_FILE = "giangvo.graphml"
...
def Astar_algorithm(pointA, pointB, time_based=False, editing=False):
    ...
    while queue:
        ...
        for neighbor, cost in graph[current_node]:
            coeff = st.session_state['edge_modifier'][(min(current_node, neighbor), max(current_node, neighbor))]
            #print(cost)
            if editing: coeff=1
...
st.title("Giảng Võ District Map")
